...
const encoreSchema = new mongoose.Schema({
  name:      { type: String, required: true },
...
const EncoreRegistration = mongoose.model("EncoreRegistration", encoreSchema);
...
app.get("/check-email", async (req, res) => {
  try {
    ...
    if (type === "innovate") {
      const existing = await InnovateRegistration.findOne({ email: { $regex: new RegExp(`^${normalizedEmail}$`, 'i') } });
      return res.json({ registered: !!existing });
    }

    if (type === "encore") {
      const existing = await EncoreRegistration.findOne({ email: { $regex: new RegExp(`^${normalizedEmail}$`, 'i') } });
      return res.json({ registered: !!existing });
    }
        ...
        <tr><td style="background:#0e1015;padding:40px;border:1px solid rgba(247,222,80,0.2);border-top:none;border-bottom:none;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="margin:24px 0;">
            <tr><td style="background:rgba(37,211,102,0.08);border:1px solid rgba(37,211,102,0.25);border-radius:12px;padding:20px 24px;text-align:center;">
              ...
              <a href="https://chat.whatsapp.com/H1yEOS9SdCILIMRjmMk5NL" style="display:inline-block;background:#25d366;color:#ffffff;font-weight:700;font-size:14px;padding:10px 24px;border-radius:8px;text-decoration:none;">Join WhatsApp Group</a>
            </td></tr>
          </table>
          <p style="font-size:14px;color:#9ca3af;line-height:1.7;margin:0;">Questions? Reach us at <a href="mailto:lexxperience01@gmail.com" style="color:#f7de50;text-decoration:none;">lexxperience01@gmail.com</a></p>
        </td></tr>
...
app.post("/encore-register", async (req, res) => {
  try {
    ...
    // Check already registered for Encore
    const existing = await EncoreRegistration.findOne({
      email: { $regex: new RegExp(`^${normalizedEmail}$`, 'i') }
    });
    if (existing) {
    ...
    const paidAmount = Number(amount || 0);
    if (paidAmount < 2000) {
      return res.status(400).json({ success: false, message: "Incorrect payment amount. Expected ₦2,000." });
    }

    const doc = await EncoreRegistration.create({
      name: name.trim(),
      email: normalizedEmail,
      reference,
      amount: paidAmount,
...
<body style="margin:0;padding:0;background:#050608;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#050608;padding:40px 20px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr><td style="background:#0e1015;padding:40px;border:1px solid rgba(247,222,80,0.2);border-top:none;border-bottom:none;">
          ...
          <h1 style="margin:0 0 8px;font-size:24px;font-weight:700;color:#f9fafb;">See you on Day 2, ${name.trim()}!</h1>
          <p style="margin:0 0 24px;font-size:15px;color:#9ca3af;line-height:1.7;">
    ...
    console.error("❌ encore-register error:", err);
    if (err.code === 11000) {
      return res.status(409).json({ success: false, message: "This email has already been registered for Lex Encore." });
    }
    return res.status(500).json({ success: false, message: "Registration failed. Please try again." });
...
// ✅ ADMIN — Get all Encore registrations
app.get("/admin/encore", async (req, res) => {
  try {
    const data = await EncoreRegistration.find({}, '-reference').sort({ createdAt: -1 });
    res.json({ success: true, data });
  } catch (err) {
    res.status(500).json({ success: false });
  }
